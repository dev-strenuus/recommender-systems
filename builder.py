...
class Builder(object):
    ...
    def __init__(self, local, testID):
        self.testID = testID
        self.local = local
        if local == True:
            self.train= pd.read_csv('input/holdout/train_holdout'+str(testID)+'.csv')
            self.test= pd.read_csv('input/holdout/test_holdout'+str(testID)+'.csv')
        else:
    ...
    def get_tracks_inside_playlist_train(self, playlist):
        return np.array(self.train[self.train['playlist_id']==playlist]['track_id'], dtype=np.int32)
    ...
    def get_durations(self):
        durations = self.tracks['duration_sec']
        return np.array(durations)
    
    def get_URM_test(self):
        self.URM_test = sparse.load_npz("input/URM_test"+str(self.testID)+".npz")
    ...
    def get_URM_train(self):
        if self.local == True:
            self.URM_train = sparse.load_npz("input/URM_train"+str(self.testID)+".npz")
        ...
        targets = self.get_ordered_target_playlists()
        targcont = 0
        cont = 0
        for playlist in range(playlistsSize):
            tracks = np.array(self.train[self.train['playlist_id']==playlist]['track_id'])
            ...
            if length > 0:
                ...
                if targcont < 5000 and playlist == targets[targcont]:
                    URM_train_values[cont:cont+length] = [0.75]*length
                    inc = 0.25/length
                    aug = 0
                    for i in range(length):
                        URM_train_values[cont+i] = URM_train_values[cont+i] + aug
                        aug = aug + inc
                    targcont = targcont +1
                else:
                    URM_train_values[cont:cont+length] = [1]*length
                cont = cont + length
        self.URM_train = sps.csr_matrix( (URM_train_values,(URM_train_row, URM_train_col)), shape=(playlistsSize, tracksSize), dtype=np.float64)
        return self.URM_train
    
    def get_URM_transpose_train(self):
        if self.local == True:
            self.URM_train_transpose = sparse.load_npz("input/URM_train_transpose"+str(self.testID)+".npz")
